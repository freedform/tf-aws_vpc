locals {
  public_subnets_mapping = {
    for i, subnet in var.public_subnets : "public_subnet_${i}" => subnet
  }
  private_subnets_mapping = {
    for i, subnet in var.private_subnets : "private_subnet_${i}" => subnet
  }
  public_az_mapping = {
    for i, subnet in var.public_subnets : "public_subnet_${i}" => data.aws_availability_zones.azs.names[i]
  }
  private_az_mapping = {
    for i, subnet in var.private_subnets : "private_subnet_${i}" => data.aws_availability_zones.azs.names[i]
  }
...
resource "aws_route_table" "private" {
  ...
  vpc_id   = aws_vpc.vpc.id
  tags = merge({
    Name = "${local.name_prefix}${replace(each.key, "subnet", "route_table")}"
  }, var.tags)
}
...
resource "aws_nat_gateway" "nat_gw" {
  ...
  tags = merge({
    Name = "${local.name_prefix}${replace(each.key, "private_subnet", "nat_gw")}"
  }, var.tags)
}
